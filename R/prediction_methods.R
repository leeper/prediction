...
#' @rdname prediction
#' @export
prediction.crch <- function(model, data = find_data(model), type = c("response", "location",
                                                                     "scale", "quantile"), ...) {
  # setup data
  data <- data
...
#' @rdname prediction
#' @export
prediction.hxlr <- function(model, data = find_data(model), type = c("class", "probability",
